...
#sko: this does not get printed in the markdown document!

#sko: put a space after your bullets. need to have a space before the bulleted list

#whats the usefullness of markdown?
- code, output, and test/notes together in one document
- knit to useful formats (pdf, html, docx)
- git friendly - version control! reproducible
- updating capabilities
- focus on output and conclusions, not code (or the other way around. flexible formatting!)
...
Dataset | Details
------------- | -------------
EPAair_PM25_NC2017_raw | data on particulate matter as a metric of air quality
ECOTOX_Neonicotinoids_Mortality_raw | data on neonicotonoid concentrations

#sko notes: good for meta data

#sko notes: 'alt' key lets you highlight a column of bullets and change them all at once
...
head(Nutrients_Raw)
class(Nutrients_Raw$sampledate)
Nutrients_Raw$sampledate <- as.Date(Nutrients_Raw$sampledate, format = "%m/%d/%y")
class(Nutrients_Raw$sampledate)
...
#Total_N_summary <- Nutrients_processed %>%
 # group_by(lakename) %>%
