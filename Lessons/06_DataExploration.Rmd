...
sko note: the best format is y/m/d
...
format(today-1, format = "%y/%b/%d/testminus1")
#the minus 1 only works with date objects
...
# checks if the date is after Dec 21, 2018 (the last possible day in the dataset). if its over, the date is actually in the 1900s, so start the string as 19, else set the string as 20. 
...
> Answer: 
remove NAs: quantitative analysis. certain statistical models can't contain NAs
keep NAs: indicates if people wanted to answer the question (social science survey). true NAs is different from missing data. if you get under a threshold, the system might report it as NA.

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # removes the entire now
# if you don't want to remove based on the entire row, can try to $
# complete.cases can be helpful to query which rows are entirely full
dim(USGS.flow.data)
...
ggplot(USGS.flow.data.complete, aes(x = discharge.mean.approval)) +
  geom_bar()
# a is approved, p is pending approval
```
...
```{r, fig.height = 3, fig.width = 4}
# shows a warning bc you don't have a great bin width
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean))

# took a discharge mean of every 10 units
ggplot(USGS.flow.data.complete) +
...
# zoomed in on everything under mean of 500
...
```{r, fig.height = 3, fig.width = 4}
# plot freqpoly on same scales 
# take measurements from 3 different columns
ggplot(USGS.flow.data.complete) +
...
# take measurements from the same column, and divide them up by a categorical variable
ggplot(USGS.flow.data.complete) +
...
# divide your data by the height of your river (i.e. gage height). the greater the depth of your river, the more water will flow through
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# the larger the amt of samples, the wider the violin
ggplot(USGS.flow.data.complete) +
