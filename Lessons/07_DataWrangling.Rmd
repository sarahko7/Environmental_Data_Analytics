...
summary(NTL.phys.data) # will give counts for categorical variables
dim(NTL.phys.data)
...
class(NTL.phys.data$depth) # continuous variable

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # need the comma bc it specifies which column you choose
...
head(NTL.phys.data.surface1) # stored the row as whatever they were in the previous dataframe
dim(NTL.phys.data.surface1)
head(NTL.phys.data.surface2) # stored the data with new rownames
...
class(NTL.phys.data$year4) #sko: bc this is an integer, it changes how you need to code
NTL.phys.1999 <- filter(NTL.phys.data, year4 == 1999)

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.Tues.1990.1999 <- filter(NTL.phys.data, year4 >= 1990 & year4 <= 1999 & lakename == "Tuesday Lake")
...
NTL.phys.data.all.temp.descending <- arrange(NTL.phys.data, desc(temperature_C))
#east long lake has highest temp
#dates are all in summer
#shallow depths have highest temps
...
#rownames = false means that the numbering in the first row should not be considered a row
```
