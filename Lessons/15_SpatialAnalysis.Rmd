...
sko: precipitation and elevation are not well represented as points, lines, polygons
...
plot(huc8_sf['SUB'],
        graticule = st_crs(4326), # add a graticule. this is a geographic grid for the background
        axes = TRUE)

#sko notes: can plot single attribute at a time
...
upperNeuse_sf <- st_read('./Data/Spatial/huc_250k_nc.shp') %>% 
  filter(HUC_NAME == 'Upper Neuse') #Filter for upper neuse
...
huc2_sf <- st_read('./Data/Spatial/huc_250k_nc.shp') %>% 
  group_by(SUB) %>%
...
### leaflet
Tip: See http://leaflet-extras.github.io/leaflet-providers/ for other basemaps. sko: add 'preview' to end of URL to get code to replace $Esri.WorldShadedRelief
...
leaflet() %>% 
  ...
  addMarkers(data=EPAair_wgs84,popup = ~as.character(`Site.Name`)) # if you click, you will get an attribute
...
huc2_UTM_nc <- st_intersection(huc2_utm,state_sf_utm) # sko: state is the cookie cutter. can do mapview(huc2_utm) to view your hucs
mapview(huc2_UTM_nc)
```
...
huc0302 <- huc2_utm %>%
  filter(SUB == '0302') # need quotes bc these are strings, not numbers
...
dim(counties_sf_utm)
dim(demog_df)
dim(counties_sf_utm_join) # only 24 columns bc GEOID and FIPS becomes a single column
...
#Combine the centroids into one featue and construct a convex hull around them. sko: like putting a rubber band around a point
...
resultMask <- st_intersects(counties_sf_utm, # sko: intersects is different from intersection
...
# sko: info from john fay - take the 1 coordinate. instead of intersecting it wiht the counties, you buffer it. then you intersect the buffer with the counties.
