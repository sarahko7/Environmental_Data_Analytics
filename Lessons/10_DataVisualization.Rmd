...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            ymin = 50, ymax = 100, fill = "yellow") + #fill the background with a color
  geom_point() +
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +
  geom_text(x = as.Date("2019-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + #added a subtitle
  scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), #bc your x is a date, don't use scale_x_continuous
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) +
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #slant the axis labels
print(O3plot7)  
...
NvsP2 <-
  ...
  xlab("Total Phosphorous (\U003BCg/L)") + #can also use paste function
...
print(NvsP2)
# can change the shape to something you can define both the fill and border of
