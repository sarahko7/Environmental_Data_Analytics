...
PeterPaul_plot2 <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C, color=depth)) + 
  geom_point(size=0.5)
...
print(NvsP) #points come up gray if they don't have an associated depth measurement
#gray band is confidence interval: 95% conf interval
#if we expect that the relationship is 16Nitrogen to Phos, and our data is mostly above the line, what does that tell us? that we are more heavily N than expected
...
ozone_vs_PM25 <- ggplot(EPAair, aes(x = PM2.5, y = Ozone, col = Site.Name)) +
  geom_point() +
  geom_smooth(method=lm)
print(ozone_vs_PM25)
