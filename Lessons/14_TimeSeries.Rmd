...
#unlike a breakpoint analysis, this will only give you 1 change point (if there is one)
# change point identified at time 35
...
# position 79, so 35+79 = point 113

# Run another Mann-Kendall for the second change point
mk.test(Peter.nutrients.surface$tp_ug[35:113]) #z=2.7 means positive relationship
...
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  geom_vline(xintercept = as.Date("1993/05/20"), linetype=2, 
                color = "#253494", size=1.5) + # pick a point any time between 5/19/1993 - 5/26/1993 
  geom_vline(xintercept = as.Date("1998/01/01"), linetype=2, 
                color = "#253494", size=1.5) +
  scale_color_manual(values = c("#7fcdbb", "#253494"))

#may need to supply origin if you are working with a date (1/1/1970 is the beginning of time
```
...
Some situations may require us to predict values for data points that fall within the time frame of our analyses but were not sampled. For instance, the `smk.test` function needs to take a time series format rather than a data frame, which cannot have any NAs. In this case, we will want to make an estimate of the missing values based on what we know about the dataset using a method called **interpolation.** There are several options for interpolation: 
  ...
  sko: aka nearest neighbor. good for distinct steps between points
  ...
  sko: connect the dots. if you are between the points, follow the line
...
USGS.flow.data.interpolated <- do.call(cbind.data.frame, USGS.flow.data.interpolated) #no more nas
names(USGS.flow.data.interpolated) <- c("Date", "Discharge") #create label names
USGS.flow.data.interpolated$Date <- as.Date(USGS.flow.data.interpolated$Date, 
                                            origin = "1970/01/01") #bring back to type 'date'

# Create a time series object
USGS.flow.data.timeseries <- ts(USGS.flow.data.interpolated$Discharge, 
                                start = c(2009, 1), frequency = 12)
#cannot click to view this item, so be careful when making this code

# Run a Seasonal Mann-Kendall test
USGS.smktest <- smk.test(USGS.flow.data.timeseries)
USGS.smktest
summary(USGS.smktest)
# the p values ask 'is there significance FOR THIS SEASON'

#march through july does not have strong significance
#winter has strong significance
```
