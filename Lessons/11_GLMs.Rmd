...
##notes:
in statistics: y = beta x + alpha + epsilon <- the error
...
y = alpha1 + alpha2 + epsilon. no beta term
...
# often strong right tails in enviro data

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less")
O3.onesample

#low p value: able to reject the null hypothesis
#t score: the further from 0, the lower your probability

```

What information does the output give us? How might we report this information in a report?

> ANSWER: The mean of ozone in 2017-18 was significantly lower than 50. one-sample ttest: t = -41.9, df = 184, 95% conf interval, p<0.0001
...
# in this case, the 95% confidence interval does not overlap 1 so these are different
...
O3.twosample
O3.twosample$p.value # this lets you call up a specific piece of your output

# reject the null bc the p value is small. the means are significantly different!
# y = blackstone (38.48) + bryson (35.18) + error

# Format as a GLM
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Site.Name)
summary(O3.twosample2)

# blackston is 38.48, for bryson city you add -3.299
...
```{r}
O3.onesample.wilcox <- wilcox.test(EPAair$Ozone, mu = 50, alternative = "less") # same as in ttest
O3.onesample.wilcox
...
#var.test only works on a pair at a time. bartlett works with more than 1 group at a time
# might not be normal, but does fulfill same variances
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name)

# Format as a GLM
PM2.5.anova <- lm(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova)
# y = blackstone(36.7) + bryson city (-4.4266) + triple oak (-3.24)
# this is saying that triple oak is 3.24 less than blackstone
# the p values from this is saying that the intercept is significantly different from 0, bryson city is significantly different from blackstone, and triple oak is significantly different from blackstone
# can't tell you anything about bryson city and triple oak
...
# tripleoak and bryson city do not have significantly different means
...
> ANSWER: (ANOVA; F = 16.16, df = 1898, p < 0.0001) describe blackstone being higher than bryson city and triple oak. bryson and triple oak are not different
...
# lake, depthid, and the interaction are all important predictors. there is an important correlation
