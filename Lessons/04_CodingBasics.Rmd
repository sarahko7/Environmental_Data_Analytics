...
vector1[3] # called 'matrix subsetting'. pick vector 1, choose third item

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # creates this vertically, then horizontally
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # creates horizontally, then vertically
matrix2
matrix3 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE, # return after comma continues the line
                  dimnames = list(c("uno", "dos", "tres", "cuatro", "cinco"), 
                                  c("un", "deux", "trois", "cat"))) #
matrix3 # this names the rows, then the columns

matrix1[4, ] # grab row 4
matrix1[ , 3] # grab column 3
matrix1[c(12, 14)] # grab elements 12 and 14
matrix1[c(12:14)] # grab elements 12 to 13
matrix1[2:4, 1:3] # grab rows 2 to 4, and columns 1-3

cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
matrix4 <- matrix(cells, nrow = 2, ncol = 2, byrow = TRUE,
  dimnames = list(rnames, cnames)) # if you only specify rows and not columns, R assumes the number of rows
matrix4

# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]] #use double square brackets for lists
...
dataframe1 <- data.frame(d,e,f) #combine vectors
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # rename column names

dataframe1[1:2] # pulls columns 1&2. if you did matrix1[1:2] it would pull elements 1&2
dataframe1[,1:2] # same as the above line
dataframe1[1:2,] # pulls the rows
dataframe1[c("ID","Passed")] # pull columns by name
dataframe1$Color # pulls all elements in the defined column
...
> ANSWER: displays column names to use

QUESTION: What happens when a comment in R is followed by "----"?

> ANSWER: makes it intos a section
...
#when items are masked, you can reinstall the packages, must do it in the correct order
