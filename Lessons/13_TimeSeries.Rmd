...
author: "Environmental Data Analytics | Sarah Ko class notes"
date: "Spring 2019"
output: pdf_document
...
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     temperature_C ~ sampledate * lakename, #if stopped here, fixed effects w interaction
                     random = ~1|Week) #add random effect
# result shows the variability in temp across a season. 14 weeks included from June-Aug
TempTest.auto
...
# intercept of fixed effect: 20.466650 C = temp at Paul lake on the first day of summer
# chose the first choice alphabetically
# sample date: 0.000099 C = amt of increase over time. 0.0001*365*32 = 1.168 degrees increase over time
# p value of sampledate:lakenamePeter Lake is high so likely no interaction
...
# if you came to the same conclusion, why include mixed effects? bc you know there is variability, so take advantage of it to create your model!

anova(TempTest.mixed, TempTest.fixed)
# The lower the AIC, the better.
# The p-value tells us whether those models have a significantly different fit
# large increase in AIC when you used fixed as opposed to mixed
...
ggplot(PeterPaul.summertemp, aes(x = sampledate, y = temperature_C, color = lakename)) +
  ...
  xlab('Sample Date') + 
  ylab('Temperature (C)') +
  ggtitle("Temperature over Summer Months") + 
  theme(legend.position="right")

# can use geomtext to add equation? some way to reproducibly do it?
