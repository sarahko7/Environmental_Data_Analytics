...
author: "Environmental Data Analytics | SKo notes"
date: "Spring 2019"
output: pdf_document
...
# numeric items missing will be filled in as NA. others like factors will be kept as blank
# R will store info on the original dataset that aren't in our subset. can use function called drop.levels
...
#if you don't want to include something with a select, put '-' before the column name
...
# to store something as a class 'date', it needs a year/month/day
...
<<<<<<< HEAD
#write.csv(NTL.phys.nutrient.data.PeterPaul, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake")
=======
...
interval(NTL.phys.nutrient.data.PeterPaul$sampledate[1], NTL.phys.nutrient.data.PeterPaul$sampledate[23372]) # not good if your data set might change over time
interval(first(NTL.phys.nutrient.data.PeterPaul$sampledate), last(NTL.phys.nutrient.data.PeterPaul$sampledate)) # this is more reproducible
```
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>% #THEN take only the surface depths
  group_by(lakename) %>% #THEN group the data by lakename
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #THEN remove the records when there are nas in temp, tn, and tp
...
<<<<<<< HEAD
# the stats are done by how you defined your groups
